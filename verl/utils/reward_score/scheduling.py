...
import random
import ast
import numpy as np  # 若 ground_truth 可能為 numpy array，需引用
...
def compute_score(solution_str, ground_truth, format_score=0.1, score=1.0):
    """
    EDF 排程問題的 reward 計算函式，分數計算邏輯如下：
    ...
    注意：針對 ground_truth['schedule'] 可能為 numpy array 的情形，做了額外的類型處理，
    避免直接與字串 "No feasible schedule" 比較時發生錯誤。
    """
    jobs = ground_truth['jobs']
    expected = ground_truth['schedule']
    
    # 若 expected 為 numpy array，轉換成 Python 型態
    ...
    if extracted.strip() == "No feasible schedule":
        # 若 expected 為字串且為 "No feasible schedule"，則滿分；否則僅得 format_score
        if isinstance(expected, str) and expected == "No feasible schedule":
            if do_print:
                print("正確判斷無可行排程")
            return score
        else:
